...
})({"72Q42":[function(require,module,exports) {
var HMR_HOST = null;
var HMR_PORT = 1234;
var HMR_SECURE = false;
var HMR_ENV_HASH = "4a236f9275d0a351";
module.bundle.HMR_BUNDLE_ID = "f9ea7086a18d6319";
"use strict";
...
},{}],"aMVE3":[function(require,module,exports) {
var _script = require("./script");
var _index = require("./pages/product/index");
var _index1 = require("./pages/user/index");

},{"./script":"j1iXy","./pages/product/index":"2jcTq","./pages/user/index":"bEs6S"}],"j1iXy":[function(require,module,exports) {
...
const toggleOption = (option, e)=>{
    const optionSelectedName = option.querySelector('.option__selected .option__name  ');
    ...
    if (e.target.closest('.option__select')) hiddenAllOption();
    if (e.target.classList.contains('option__name')) optionSelectedName.textContent = e.target.textContent;
    e.stopPropagation();
};
const toggleSortIcon = (e)=>{
    $$('.table__icon-sort--active').forEach((icon)=>{
        if (icon.closest('.table__heading') !== e.currentTarget) icon.classList.remove('table__icon-sort--active');
    });
    const iconUp = e.currentTarget.querySelector('i:nth-child(1)');
    const iconDown = e.currentTarget.querySelector('i:nth-child(2)');
    if (!iconUp.classList.contains('table__icon-sort--active') && !iconDown.classList.contains('table__icon-sort--active')) iconUp.classList.add('table__icon-sort--active');
    ...
    } else if (iconDown.classList.contains('table__icon-sort--active')) iconDown.classList.remove('table__icon-sort--active');
};
...
const togglePopup = ()=>{
    ...
    if (checkboxesChecked.length === 0) {
        ...
        $('.popup--confirm .popup__message').textContent = ` Bạn có chắc muốn xóa ${checkboxesChecked.length} mục này`;
    }
};
document.addEventListener('click', (e)=>{
    hiddenAllOption();
    if (e.target.closest('.option')) toggleOption(e.target.closest('.option'), e);
    if (e.target.closest('.toggle-switch')) e.target.closest('.toggle-switch').classList.toggle('toggle-switch--checked');
});
$$('.popup').forEach((popup)=>{
    popup.addEventListener('click', (e)=>{
        if (e.target.closest('.btn') || e.target.closest('.popup__icon-close')) togglePopup();
    });
});
...
if (window.location.pathname === '/') $('.nav__item:nth-child(1)').classList.add('nav__item--active');
if (window.location.pathname.startsWith('/products') || window.location.pathname.startsWith('/categories')) {
    $('.nav__item:nth-child(2)').classList.add('nav__item--active');
    $('.sub-navbar:nth-child(3)').classList.remove('display-n');
}
if (window.location.pathname.startsWith('/products')) $('.sub-nav [href="/products"]').classList.add('sub-nav__item--active');
if (window.location.pathname.startsWith('/categories')) $('.sub-nav [href="/categories"]').classList.add('sub-nav__item--active');
// NOTIFICATION
const notifications = $$('.notification');
notifications.forEach((notification)=>{
    notification.addEventListener('click', (e)=>{
        if (e.target.classList.contains('notification__icon-close')) notification.classList.add('hidden');
    });
});
// SORT IN TABLE
[
...
(async ()=>{
    ...
    _utils.updatePaginationInfo(allProducts.length);
    _productView.renderRows(allProducts.slice(0, 9));
})();
const getProducts = ()=>{
...
const createFormData = async ()=>{
    ...
    const idCat = (await _category.getAllCategory(`?name=${$('#category').textContent}`))[0]._id;
    ...
    const allowSell = $('#allowSell').classList.contains('toggle-switch--checked');
...
$('.form')?.addEventListener('click', async (e)=>{
    if (e.target.closest('#add')) {
        ...
        e.preventDefault();
        try {
            _utils.renderSpinner($('.form'));
            _productView.removeFormError();
            const formData = await createFormData();
            await _product.createProduct(formData);
            _productView.resetForm();
            _utils.removeSpinner($('.form'));
            _utils.renderNotification('success', `Thêm sản phẩm thành công`);
            allProducts = await _product.getAllProduct();
            ...
            messages.forEach((message)=>_productView.renderFormError(message)
            );
        }
    }
    if (e.target.closest('#modify')) {
        ...
        try {
            _utils.renderSpinner($('.form'));
            const formData = await createFormData();
            const prod = await _product.getAllProduct(`?title=${currentTitle}`);
            const newProd = await _product.updateProduct(prod[0]._id, formData);
            currentTitle = newProd.title;
            _utils.removeSpinner($('.form'));
            _utils.renderNotification('success', 'Lưu thành công');
            allProducts = await _product.getAllProduct();
            _productView.renderRows(getProducts());
        } catch (err) {
            _utils.removeSpinner($('.form'));
            _utils.renderNotification('error', 'Lưu thất bại. Vui lòng thử lại');
        }
    }
});
$('#delete')?.addEventListener('click', async (e)=>{
    ...
    try {
        ...
        await Promise.all([
            ...
            await _product.deleteProduct(title);
        }));
        allProducts = await _product.getAllProduct();
        _productView.renderRows(getProducts());
        _utils.renderNotification('success', `Đã xóa ${checkboxesChecked.length} mục thành công`);
        _utils.updatePaginationInfo(allProducts.length, page, limit);
    } catch (err) {
        _utils.removeSpinner($('.table__body'));
        _utils.renderNotification('error', 'Xóa thất bại');
    }
});
...
$('.table')?.addEventListener('click', async (e)=>{
    if (e.target.closest('#details')) {
        ...
        currentTitle = e.target.closest('.table__row').querySelector('.table__data:nth-child(2)').textContent;
        const prod = (await _product.getAllProduct(`?title=${currentTitle}`))[0];
        const cat = await _category.getCategory(prod.category);
        prod.category = cat.name;
        _utils.removeSpinner($('.form'));
        _productView.renderDetails(prod);
    }
});
...
[
    ...
    $(`.table__heading:nth-child(${column})`)?.addEventListener('click', async (e)=>{
        const iconActive = e.currentTarget.querySelector('.table__icon-sort--active');
        if (iconActive) {
...
$('.paginate')?.addEventListener('click', async (e)=>{
    ...
    const infoPage = _utils.getInfoPage();
    if (pageEl) {
...
$('.filter-option-list').addEventListener('click', async (e)=>{
    e.preventDefault();
    if (e.target.classList.contains('option__name') && !e.target.closest('.option__selected') || e.target.classList.contains('btn')) {
        const el = e.target.closest('.filter-option');
        if (el.dataset.categoryFilter) {
            const cat = e.target.textContent;
            if (cat === 'Thể loại') queryString = queryString.replace(/category=.*?&/, '');
            else {
                const catId = (await _category.getAllCategory(`?name=${cat}`))[0]._id;
                if (!queryString) queryString = '?';
                if (queryString.search('category') !== -1) queryString = queryString.replace(/(?<=category=)(.*?)(?=&)/, catId);
                else queryString += `category=${catId}&`;
            }
        }
        if (el.dataset.allowSellFilter) {
            const allowSell = e.target.textContent;
            if (allowSell === 'Trạng thái') queryString = queryString.replace(/allowSell=.*?&/, '');
            else {
                const isAllowSell = e.target.textContent.startsWith('Đ');
                if (queryString.search('allowSell') !== -1) queryString = queryString.replace(/(?<=allowSell=)(.*?)(?=&)/, isAllowSell);
        ...
        if (el.dataset.listPriceFilter) {
            ...
            if (!from || !to) {
                ...
                queryString = queryString.replace(/(?<=listPrice\[gte\]=)(.*?)(?=&)/, from);
                queryString = queryString.replace(/(?<=listPrice\[lte\]=)(.*?)(?=&)/, to);
        ...
        if (el.dataset.originalPriceFilter) {
            ...
            if (!from || !to) {
                ...
                queryString = queryString.replace(/(?<=originalPrice\[gte\]=)(.*?)(?=&)/, from);
                queryString = queryString.replace(/(?<=originalPrice\[lte\]=)(.*?)(?=&)/, to);
            } else queryString += `originalPrice[gte]=${from}&originalPrice[lte]=${to}&`;
        }
        page = 1;
        sort = '';
        _utils.renderSpinner($('.table__body'));
...
},{"../category/category":"112Lo","./productView":"jmLLq","./product":"im95S","../../utils":"9Loev"}],"112Lo":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "getAllCategory", ()=>getAllCategory
);
...
},{}],"jmLLq":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "renderRow", ()=>renderRow
);
parcelHelpers.export(exports, "renderRows", ()=>renderRows
);
parcelHelpers.export(exports, "renderCreate", ()=>renderCreate
);
parcelHelpers.export(exports, "renderDetails", ()=>renderDetails
);
parcelHelpers.export(exports, "renderFormError", ()=>renderFormError
);
parcelHelpers.export(exports, "removeFormError", ()=>removeFormError
);
parcelHelpers.export(exports, "activeHeading", ()=>activeHeading
);
parcelHelpers.export(exports, "activeContent", ()=>activeContent
);
...
const activeContent = ()=>{
    $('.form-body__content--active').classList.remove('form-body__content--active');
    $$('.form-header__heading').forEach((headerHeading, index)=>{
        if (headerHeading.classList.contains('form-header__heading--active')) $$('.form-body__content')[index].classList.add('form-body__content--active');
    });
};
const resetForm = ()=>{
    $('.form').reset();
    if ($('.form fieldset')) $('.form .form-body').innerHTML = $('.form fieldset').innerHTML;
...
const renderFormError = (message)=>{
    ...
    if (!formHeading.querySelector('.form-header__error')) formHeading.insertAdjacentHTML('beforeend', `<div class="form-header__error"></div>`);
};
...
const getHTMLProductRow = (prod)=>{
    const allowSell = prod.allowSell ? `primary">Đang bán` : `secondary">Không bán`;
...
const renderRows = async (products)=>{
    try {
        ...
        _utils.renderNotification('error', 'Hiển thị dữ liệu thất bại! Hãy thử tải lại trang');
    }
};
...
},{"../category/category":"112Lo","../../utils":"9Loev","@parcel/transformer-js/src/esmodule-helpers.js":"ciiiV"}],"9Loev":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "gotoPage", ()=>gotoPage
);
parcelHelpers.export(exports, "updatePaginationInfo", ()=>updatePaginationInfo
);
parcelHelpers.export(exports, "getInfoPage", ()=>getInfoPage
);
parcelHelpers.export(exports, "renderNotification", ()=>renderNotification
);
parcelHelpers.export(exports, "renderSpinner", ()=>renderSpinner
);
parcelHelpers.export(exports, "removeSpinner", ()=>removeSpinner
);
const $ = document.querySelector.bind(document);
const $$ = document.querySelectorAll.bind(document);
const gotoPage = (page)=>{
...
const updatePaginationInfo = (quantity, page = 1, limit = 10)=>{
    ...
    if (spans.length > 0) {
        ...
        if (page * limit > quantity) spans[1].textContent = quantity;
        else spans[1].textContent = page * limit;
        spans[2].textContent = quantity;
    }
    let html = '<i class="fas fa-chevron-left"></i>';
    for(let i = 1; i <= maxPage; i += 1)if (i === page) html += `<span class="current-page">${i}</span>`;
...
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "getAllProduct", ()=>getAllProduct
);
...
},{"axios":"1IeuP","@parcel/transformer-js/src/esmodule-helpers.js":"ciiiV"}],"bEs6S":[function(require,module,exports) {
...
$('#login')?.addEventListener('submit', async (e)=>{
    e.preventDefault();
    try {
        _userView.removeErrorLogin();
        const email = $('#email').value;
        const password = $('#password').value;
        const status = await _user.login(email, password);
        if (status === 'success') window.location.assign('/');
    } catch (err) {
        _userView.renderErrorLogin(err);
    }
});
$('#logout')?.addEventListener('click', async ()=>{
    try {
        const status = await _user.logout();
        if (status === 'success') window.location.assign('/login');
    } catch (err) {
        _userView.renderErrorLogout();
    }
});

},{"./user":"kgZPr","./userView":"UEEIJ"}],"kgZPr":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
},{"axios":"1IeuP","@parcel/transformer-js/src/esmodule-helpers.js":"ciiiV"}],"UEEIJ":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "renderErrorLogin", ()=>renderErrorLogin
);
parcelHelpers.export(exports, "removeErrorLogin", ()=>removeErrorLogin
);
parcelHelpers.export(exports, "renderErrorLogout", ()=>renderErrorLogout
);
var _utils = require("../../utils");
const $ = document.querySelector.bind(document);
const $$ = document.querySelectorAll.bind(document);
...
const renderErrorLogout = ()=>{
    _utils.renderNotification('error', 'Đăng xuất thất bại. Vui lòng thử lại');
};

},{"../../utils":"9Loev","@parcel/transformer-js/src/esmodule-helpers.js":"ciiiV"}]},["72Q42","aMVE3"], "aMVE3", "parcelRequiree332")
